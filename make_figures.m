...
clims_studies                = [2   2   10  10  10 10 30 10  10 10 2];
exper_designs_studies_exp_id = [100 100 100 100 6  6  2  100 4  11 100];
...
for smoothing_id = range_smoothings
   ...
   figure('rend', 'painters', 'pos', fig_size);
   study_id_plot            = 0;
   for study_id = range_studies
      study         = studies{study_id};
      study_as_text = strrep(study, '_', ' ');
      if any(strcmp(studies_out, study))
         ...
         %h2    = plot(x_axis, squeeze(y_axes(1,1,:)), colors(1)); hold on;
         ...
         htitle = title(study_as_text, 'interpreter', 'none');
   ...
   plot(1);
   set(gca, 'visible', 'off');
   hlegend = legendflex([h1 h3 h4 h5 h6 h15 h16 h0 h18], {'AFNI', 'FSL: 1/100', 'FSL: 1/(off+on times)', 'SPM: 1/128', 'SPM: 1/(10+off+on times)', 'Expected', '95% CI', 'Exper design', sprintf('x : \t Block  x s off +  x s on')}, 'ref', fig_ref, 'anchor', [4 8], 'buffer', [-180 1], 'box', 'off', 'FontSize', 7, 'nrow', 9);
   ...
   plot(1);
   set(gca, 'visible', 'off');
   hlegend = legendflex([h1 h3 h4 h5 h6 h0 h18], {'AFNI', 'FSL: 1/100', 'FSL: 1/(off+on times)', 'SPM: 1/128', 'SPM: 1/(10+off+on times)', 'Exper design', sprintf('x : \t Block  x s off +  x s on')}, 'ref', fig_ref, 'anchor', [4 8], 'buffer', [-180 1], 'box', 'off', 'FontSize', 7, 'nrow', 7);
   set(hlegend, 'FontSize', 7);
...
for software_id = range_softwares
   ...
   parfor smoothing_id = range_smoothings
      ...
      for study_id = range_studies
         ...
         if any(strcmp(studies_out, study)) || strcmp('FCP_Beijing', study)
            continue
         else
            study_id_plot = study_id_plot + 1;
            ...
            title([software ': ' study_as_text], 'interpreter', 'none', 'FontSize', 6);
