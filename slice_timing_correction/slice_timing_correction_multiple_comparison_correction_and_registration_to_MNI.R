...
for (package in packages) {
   ...
   for (smoothing in smoothings[4]) {
      ...
      path = paste0(path_output_top, "/", package, "/smoothing_", smoothing, "/exper_design_", exper_design, "/HRF_", HRF_model, "_slice_timing_correction/", subject)
      setwd(path)

      system("mv standardized_stats standardized_stats_2018_08_03")
