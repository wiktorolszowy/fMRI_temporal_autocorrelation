...
#for ((smoothing_id=0; smoothing_id<${#smoothings[@]}; smoothing_id++)) {
   ...
   if [ "$study" == CRIC_checkerboard ]; then
      exper_design_id=3
      tpattern=alt+z2
   elif [ "$study" == CamCAN_sensorimotor ]; then
      exper_design_id=16
      tpattern=seqminus
   fi
   ...
   cd ${path_output}/smoothing_${smoothing}/exper_design_${exper_design}/HRF_${HRF_model}_slice_timing_correction
